...
@WebMvcTest(
        controllers = {UserController.class, ProductController.class},   //test할 controller
        excludeFilters = {
                @ComponentScan.Filter(
                        type = FilterType.ASSIGNABLE_TYPE,
                        classes = WebSecurityConfig.class    //보안 관련 config 클래스
                )
        }
)
class UserProductMvcTest {
    ...
    private Principal mockPrincipal;   //가짜 인증에 필요
...
//    KakaoService kakaoService;
    ...
    public void setup() {
        mvc = MockMvcBuilders.webAppContextSetup(context)
                .apply(springSecurity(new MockSpringSecurityFilter()))   //우리가 만든 가짜 필터
                .build();
    }
    ...
    void test1() throws Exception {
        // when - then
        mvc.perform(get("/api/user/login-page"))   //http 메서드와 urlpath
                .andExpect(status().isOk())                 //예측한 status
                .andExpect(view().name("login"))   //위의 perfome 실행 후 반환되는 html파일
                .andDo(print());
    }
